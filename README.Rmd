...
Inspect each variable and learn about its scale and derivation here: <br/>
...
## summarize_dyads()
This last step will append two metrics of alignment to each dyad (AUC and Spearman R) for every variable of interest you specified. For example, summarize_dyads will append AUC and Spearman values for hostility (if that's what you're interested in) to MaryandMikeFirstDate. <br/>
<br/>
Link here to read more about what AUC and Spearman mean in the context of alignment (or look at the figure above for an illustration):  LINK TO METHOD

AUC will appear in the dataframe with a prefix "auc_". Spearman's correlation coefficient will appear with a prefix ("S_rho_") per variable per dyad. <br/>
