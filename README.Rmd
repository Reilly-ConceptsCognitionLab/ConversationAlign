...
knitr::kable(head(NurseryRhymes_Aligned, 20), format = "simple")
```
...
NurseryRhymes_Analytics <-  corpus_analytics(dat_prep=NurseryRhymes_Aligned)
knitr::kable(head(NurseryRhymes_Analytics, 15), format = "simple")
```
...
```{r, eval=T, warning=F, message=F}
MarySumDat <- summarize_dyads(df_prep = NurseryRhymes_Aligned) 
colnames(MarySumDat)
knitr::kable(head(MarySumDat, 15), format = "simple")
```
<br>
<br>
<br>
