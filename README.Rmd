...
ConversationAlign is a tool for computing main effects and alignment dynamics in 2-person conversation transcripts. At present, ConversationAlign is capable of analyzing synchrony across more than 30 different affective, lexical, and semantic dimensions.  Email [Jamie Reilly](mailto:reillyj@temple.edu) to suggest more. In the demo to follow, we will analyze a real conversation transcript included in the package. This transcript reflects a classic interview between American radio host, Terry Gross, and comedian, Marc Maron, first broadcast on National Public Radio (2013). Here are the first 10 lines.
...
2. ``Paricipant_ID`` column header delineating who is producing the text on a given line (variable as factor) <br>
...
- You can specify NOT to apply cleaning operations such as stopword removal or lemmatization to your raw text. However, some transformations are obligatory, including the following: <br>
...
<span style="color: darkred;">Arguments to ``clean_dyads`` include:</span> <br>
...
4) **which_stopwords**= quoted argument specifying stopword list, options include ``none``, ``MIT_stops``, ``SMART``, ``CA_OriginalStops``, or ``Temple_Stopwords25``. Default is ``Temple_Stopwords25``. ``CA_OriginalStops`` is the stopword list originally bundled with ``ConversationAlign`` during its early development. <br>

```{r, eval=T, message=F, warning=F}
Maron_Cleaned <- clean_dyads(read_ts_df=Maron_Prepped, lemmatize=TRUE, omit_stops=T, which_stoplist="Temple_stops25")

knitr::kable(head(Maron_Cleaned, 40), format = "pipe")
```
<br>
